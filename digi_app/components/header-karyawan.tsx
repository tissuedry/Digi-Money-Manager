...
import { Bell, Menu, Check, ArrowRight, Loader2, LogOut } from "lucide-react";
...
export default function Header({ onOpenSidebar }: HeaderProps) {
  ...
  return (
    <header className="h-20 flex items-center justify-between px-8 border-b border-stone-200 bg-[#f9f8f4] shrink-0 sticky top-0 z-30 w-full">
      {/* Box Kiri Header */}
      ...
      <div className="flex items-center gap-4 relative">
        {/* Tombol Notifikasi & Dropdown Container */}
        <div ref={dropdownRef} className="relative">
          ...
          >
            ...
            {unreadCount > 0 && (
              <span className="absolute -top-1.5 -right-1.5 min-w-[18px] h-[18px] px-1 bg-[#a63737] text-white text-[10px] font-bold flex items-center justify-center rounded-full border-2 border-[#f9f8f4]">
                {unreadCount}
              </span>
            )}
          </button>

          {/* Notifikasi Dropdown Popover */}
          {isNotificationsOpen && (
            <div className="absolute right-0 mt-3 w-[360px] sm:w-[420px] bg-[#fdfcf9] border border-stone-200 rounded-2xl shadow-xl z-50 overflow-hidden flex flex-col max-h-[500px] animate-in fade-in slide-in-from-top-3 duration-250">
              {/* Header Dropdown */}
              <div className="px-5 py-4 border-b border-stone-200/80 bg-[#f5f4ef] flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[11px] text-stone-500 mt-0.5 font-medium">
                    {unreadCount} belum dibaca · {profile?.role || "Karyawan"}
                  </p>
                </div>
                <button
              ...
              <div className="flex-1 overflow-y-auto divide-y divide-stone-100 max-h-[350px]">
                {notifications.length > 0 ? (
                  notifications.map((item) => (
                    ...
                    >
                      {/* Left Icon (Circular Green Check) */}
                      ...
                      {/* Right Indicator (Blue Dot for Unread) */}
                      {!item.dibaca && (
        ...
        <div ref={profileRef} className="relative">
          ...
          >
            <div className="w-8 h-8 bg-[#c2e0d1] text-[#117a5b] font-bold text-[13px] flex items-center justify-center rounded-full shadow-inner">
              {getInitials(profile?.nama || "Alif Ihsan")}
            </div>
            <div className="flex flex-col pr-2 select-none">
              <span className="text-[12px] font-bold text-stone-800 leading-tight">
                {profile?.nama || "Alif Ihsan"}
              </span>
